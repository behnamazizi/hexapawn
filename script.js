...
function isWon() {
    ...
    if (winner.length > 0) {
        ...
        winner === 'computer' ?
        (winnerpos = snapshot[snapshot.length - 1], snapshot = '', points[0]++, new Audio('lost.wav').play()):
        (badIdeas.push(snapshot), snapshot = '', points[1]++, new Audio('win.wav').play());
